...
export const MemberView: FC<{ user: any; points: number }> = ({
  user,
  points,
}) => {
  ...
  return (
    <Box className="bg-gray-100 min-h-screen pb-10">
      ...
      <Box
        className={`m-4 p-5 rounded-2xl text-white shadow-xl transition-all ${
          activeWallet === "main" ? "bg-orange-500" : "bg-blue-600"
        }`}
      >
        <Box
          flex
        ...
        >
          <Box>
            <Text size="small" className="opacity-80 uppercase">
              {activeWallet === "main" ? "Member Rank" : "Promo Status"}
            </Text>
          ...
          <Box className="text-right">
            ...
            <Text bold size="xLarge">
              {activeWallet === "main" ? points.toLocaleString() : "0"}
            </Text>
          </Box>
        </Box>
        <Progress
          completed={
            activeWallet === "main"
              ? Math.min(
                  100,
                  (points / calculateMemberRankInfo(points).target) * 100
                )
              : 0
          }
          maxCompleted={100}
      ...
      <Box className="m-4 p-4 bg-white rounded-xl shadow-sm border border-orange-100">
        ...
        <Box className="bg-orange-50 p-3 rounded-lg border border-dashed border-orange-300 flex justify-between items-center">
          <Box>
            <Text size="xxSmall" className="text-gray-500 uppercase">
              Mã giới thiệu của bạn
            </Text>
            <Text
              bold
              size="large"
              className="text-orange-600 letter-spacing-2"
            >
              {referralCode}
            </Text>
          </Box>
        ...
        <Text size="xxSmall" className="mt-2 text-gray-400 italic">
          * Chia sẻ mã này cho bạn bè khi đăng ký để nhận thêm điểm thưởng ưu
          đãi.
        </Text>
      </Box>
      ...
      <Modal
        ...
        title={
          activeWallet === "main" ? "Lịch sử Ví Tích Điểm" : "Lịch sử Ví Ưu Đãi"
        }
      ...
      >
        <Box p={4} style={{ maxHeight: "60vh", overflowY: "auto" }}>
          {historyLoading ? (
            ...
            historyList.map((item, idx) => (
              ...
              >
                ...
                <Text
                  size="small"
                  className={
                    item.type === "plus" ? "text-green-600" : "text-red-600"
                  }
                >
                  {item.type === "plus" ? "+" : "-"}
            ...
            <Text size="small" className="text-center text-gray-400">
              Không có dữ liệu.
            </Text>
          )}
        </Box>
